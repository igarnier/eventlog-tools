...
module MH_test () = struct
  ...
  let () =
    ...
    let open Plot in
    let plot  =
      plot2 ~xaxis:"x" ~yaxis:"freq" ~title:( "mcmc result") @@
      [ Histogram.hist
          ~points:(Data.of_seq @@ Seq.map r1 (Array.to_seq float_data))
          ~binwidth:0.1 ()
      ]
    in
    Plot.run ~target:(Plot.png ~png_file:"mcmc.png" ()) ~plot:plot Plot.exec
end
