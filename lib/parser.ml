...
let caml_trace_version e =
  match e with Le -> LE.any_int16 | Be -> BE.any_int16

let stream_id e =
  let f = match e with Le -> LE.int16 | Be -> BE.int16 in
  f 0x0

let ( >>? ) v f = match v with Ok v -> f v | Error (`Msg msg) -> fail msg
...
let parse_event e =
  ...
  let entry_event =
    int32 0x0l *> any_int16 >>= fun i ->
    phase_of_int i >>? fun phase -> return (Entry { phase })
  in
  let exit_event =
    int32 0x1l *> any_int16 >>= fun i ->
    phase_of_int i >>? fun phase -> return (Exit { phase })
  in
  let counter_event =
    int32 0x2l *> any_int64 >>= fun count ->
    any_int16 >>= fun i ->
    gc_counter_of_int i >>? fun kind ->
    return (Counter { count = Int64.to_int count; kind })
  in
  let alloc_event =
    int32 0x3l *> any_int64 >>= fun count ->
    any_int8 >>= fun i ->
    alloc_bucket_of_int i >>? fun bucket ->
    return (Alloc { count = Int64.to_int count; bucket })
  in
  let flush_event =
    int32 0x4l *> any_int64 >>= fun duration ->
    return (Flush { duration = Int64.to_int duration })
  in
  let event_header =
    any_int64 >>= fun timestamp ->
    any_int32 >>= fun pid -> return (timestamp, pid)
  in
  let event_parser =
    entry_event <|> exit_event <|> alloc_event <|> counter_event <|> flush_event
  in
...
let rec decode d =
  ...
  | Unbuffered.Done (i, v) ->
      d.off <- d.off + i ;
      (match v with
      | Header { endianness; _ } -> d.parser <- parse_event endianness
      | _ -> ()) ;
      d.state <- Unbuffered.parse d.parser ;
      `Ok v
  | Fail (_, _, msg) ->
      if d.complete = Unbuffered.Complete && d.off == d.len then `End
      else
        let err =
          Printf.sprintf
...
let src d src src_off src_len complete =
  ...
  d.len <- dst_len ;
  d.complete <-
    (if complete then Unbuffered.Complete else Unbuffered.Incomplete)
